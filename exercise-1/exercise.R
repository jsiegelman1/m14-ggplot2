...
#print(ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = carat, y = price)))

# Draw the same plot as above, but color each of the points based on their clarity.
#print(ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = carat, y = price, color=clarity)))


# Draw the same plot as above, but for the entire `diamonds` data set. Note this may take
# a few seconds to generate.
#print(ggplot(data = diamonds) + geom_point(mapping = aes(x = carat, y = price, color=clarity)))


# Draw another scatter plot for `diamonds.sample` of price (y) by carat (x),
# but with all of the dots colored "blue".
# Hint: you'll need to set the color channel, not map a value to it!
#print(ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = carat, y = price), color = "blue"))


# Draw a scatter plot for `diamonds.sample` of `price` by `carat`, where each
# point has an aesthetic _shape_ based on the diamond's `cut`.
print(ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = carat, y = price, shape = cut)))


# Draw a scatter plot for `diamonds.sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*
print(ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = cut, y = carat, size = price)))


# Try coloring the above plot based on the diamond's price!
print(ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = cut, y = carat, size = price, color = price)))
...
print(ggplot(data = diamonds.sample) + geom_line(mapping = aes(x = carat, y = price, color = cut)))
